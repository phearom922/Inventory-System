...
function ProductManagement() {
  const { branchFilter } = useAuth(); // รับ branchFilter จาก Context หรือ props
  ...
  useEffect(() => {
    const branchId = Array.isArray(branchFilter) ? branchFilter[0] : branchFilter;
    if (!branchId) {
      console.warn('No branchId available yet, skipping fetch...');
      return;
    }
    ...
    const fetchProducts = async () => {
      try {
        ...
        const response = await getProducts({ branchId });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const dataToSend = { ...formData, branchId: branchFilter[0] || branchFilter }; // ใช้ branchId แรก
      if (editProductId) {
